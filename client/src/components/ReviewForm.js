...
class ReviewForm extends React.Component {
  ...
  componentDidMount() {
      if (this.props.edit) {
      axios.get(`/api/products/${this.props.location.state.product_id}/reviews/${this.props.match.params.id}`)
        .then(res => {
          // the long way
  ...
  handleSubmit = (e) => {
    ...
    if (this.props.edit) {
      ...
      const { location: { state: { product_id } } } = this.props
      axios.put(`/api/products/${product_id}/reviews/${id}`, { ...this.state })
        .then(res => {
          this.props.history.goBack()
        })
    } else {
      const { product_id } = this.props
      axios.post(`/api/products/${product_id}/reviews`, { ...this.state })
